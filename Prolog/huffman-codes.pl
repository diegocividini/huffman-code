...
% Main predicate to generate a list of symbols and their weights 
...
% Main predicate to generate the Huffman tree 
% hucodec_generate_huffman_tree/2 SymbolsAndWeights HuffmanTree
...
decode_bits([], _, _, []).
    decode_bits([Bit|Bits], node([Symbol], _, nil, nil), HuffmanTree,
		[Symbol|RestMessage]) :-
